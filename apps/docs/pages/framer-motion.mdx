...
import { ActiveSectionInfoMotionValue } from '../components/grouped/ActiveSectionInfoMotionValue';
...
import { SectionScrollMotionValue } from '../components/single/SectionScrollMotionValue';
...
<SectionScrollMotionValue className="border-green-400 bg-green-200 dark:border-green-700 dark:bg-green-800" />

## Multiple Sections

### Example

You can simply the example in [track grouped sections](/track_grouped_sections) and use `useActiveSectionMotionValue()` to get the active section information and the updated `MotionValue` based on the scroll progress:
