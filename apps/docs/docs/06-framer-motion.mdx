...
import { ActiveSectionInfoMotionValue } from '../src/components/grouped/ActiveSectionInfoMotionValue';
...
import { SectionScrollMotionValue } from '../src/components/single/SectionScrollMotionValue';
...
You can simply the example in [track grouped sections](./03-track_grouped_sections.mdx) and use `useActiveSectionMotionValue()` to get the active section information and the updated `MotionValue` based on the scroll progress:
