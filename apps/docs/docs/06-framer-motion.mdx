...
You can simply the example in [track grouped sections](/docs/track_grouped_sections) and use `useActiveSectionMotionValue()` to get the active section information and the updated `MotionValue` based on the scroll progress:
