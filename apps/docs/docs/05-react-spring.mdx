...
import { SectionScrollSpring } from '../src/components/single/SectionScrollSpring';
...
You can simply the example in [track grouped sections](./03-track_grouped_sections.mdx) and use `useActiveSectionSpring()` to get the active section information and the updated `SpringValue` based on the scroll progress:
